...
def get_model(summary=False, backend='tf'):
    ...
    model.add(MaxPooling3D(pool_size=(1, 2, 2), strides=(1, 2, 2),
                           border_mode='valid', name='pool1', trainable=False))
    # 2nd layer group
    model.add(Convolution3D(128, 3, 3, 3, activation='relu',
                            border_mode='same', name='conv2', trainable=False))
    model.add(MaxPooling3D(pool_size=(2, 2, 2), strides=(2, 2, 2),
                           border_mode='valid', name='pool2', trainable=False))
    # 3rd layer group
    model.add(Convolution3D(256, 3, 3, 3, activation='relu',
                            border_mode='same', name='conv3a', trainable=False))
    model.add(Convolution3D(256, 3, 3, 3, activation='relu',
    ...
    model.add(Flatten())
    # FC layers group
    model.add(Dense(4096, activation='relu', name='gfc6'))
    model.add(Dropout(.5))
    model.add(Dense(4096, activation='relu', name='gfc7'))
    model.add(Dropout(.5))
    ##model.add(Dense(487, activation='softmax', name='fc8')

    model.add(Dense(32, activation='softmax', name='gs32'))
